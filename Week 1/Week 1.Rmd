...
samp1 %>%
  summarise(mu_sample = mean(area), pop_med_s = median(area), 
            ...
            pop_q1_s = quantile(area, 0.25),  # first quartile, 25th percentile
            pop_q3_s = quantile(area, 0.75))  # third quartile, 75th percentile
```
...
# type your code for the Exercise here, and Run Document
sample_means50 %>%
  summarise(x_bar_mean = mean(x_bar), pop_med = median(x_bar), 
            SE = sd(x_bar), pop_iqr = IQR(x_bar),
...
sample_means15 <- ames %>%
                    ...
                    summarise(x_bar_price = mean(price))
ggplot(data = sample_means15, aes(x = x_bar_price)) +
  geom_histogram(binwidth = 1000)
