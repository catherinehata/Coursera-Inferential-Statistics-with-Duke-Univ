...
Percent_atheist <- (table(us12$response == 'atheist')[2])/nrow(us12)
```
...
inference(y = response, x = as.factor(year), data = spain, statistic = "proportion", type = "ht", null = 0, alternative = "twosided", method = "theoretical", success = 'atheist')
```
...
inference(y = response, x = as.factor(year), data = us, statistic = "proportion", type = "ht", null = 0, alternative = "twosided", method = "theoretical", success = 'atheist')
```
...
# Take the ME from previous, divide that by a little bit more than 3, which is the same as multiplying n by >9 (since n is under the square root)
d <- data.frame(p <- seq(0, 1, 0.01))
n <- 9604
d <- d %>%
  mutate(me = 1.96*sqrt(p*(1 - p)/n))
