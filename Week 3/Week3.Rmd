...
row_to_keep = which(!(nc$habit == 'NA'))
nc_n = nc[row_to_keep,]
smoker <- table(nc_n$habit == 'smoker')[2]
nonsmoker <- table(nc_n$habit == 'nonsmoker')[2]
id <- rep(c('smoker','nonsmoker'), c(smoker, nonsmoker))
weight_s <- nc_n$weight[nc_n$habit == 'smoker']
weight_ns <- nc_n$weight[nc_n$habit == 'nonsmoker']
d <- data.frame(y = c(weight_s, weight_ns), id = factor(id))
boxplot(y ~ id, data = d)
```
...
younger_mom <- nc[nc$mature == 'younger mom',]
mature_mom <- nc[nc$mature == 'mature mom',]
range_y <- range(younger_mom$mage)
range_m <- range(mature_mom$mage)
```
